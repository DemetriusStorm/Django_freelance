...
ORDER_RESPONCE_STATUS_CHOICES = tuple(map(lambda x: (x.value, x.name), OrderResponceStatus))


class Tag(models.Model):
    tag = models.CharField(max_length=255)
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='tag_creator')
...
class Order(models.Model):
    ...
    executor = models.ForeignKey(Executor, related_name='order_executor', on_delete=models.CASCADE, null=True)
...
class OrderTag(models.Model):
    tag = models.ForeignKey(Tag, on_delete=models.CASCADE, related_name='order_tag_key')
    order = models.ForeignKey(Order, on_delete=models.CASCADE, related_name='order_tag')
    created_at = models.DateTimeField(auto_now_add=True)
...
class OrderResponce(models.Model):
    order = models.ForeignKey(Order, on_delete=models.CASCADE, related_name='order_responce')
    executor = models.ForeignKey(Executor, on_delete=models.CASCADE)
    status = models.IntegerField(choices=ORDER_RESPONCE_STATUS_CHOICES, default=0)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
